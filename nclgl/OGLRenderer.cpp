...
OGLRenderer::OGLRenderer(Window &window) {
  ...
  if (!(PixelFormat = ChoosePixelFormat(
    ...
    std::cout
        << "OGLRenderer::OGLRenderer(): Failed to choose a pixel format!\n";
    return;
  }
  ...
  if (!(tempContext = wglCreateContext(
    ...
    std::cout
        << "OGLRenderer::OGLRenderer(): Cannot create a temporary context!\n";
  ...
  if (!gladLoadGL()) {
    std::cout
        << "OGLRenderer::OGLRenderer(): Cannot initialise GLAD!\n"; // It's all
                                                                    // gone
                                                                    // wrong!
    return;
  }
  ...
  char *ver =
      (char *)glGetString(GL_VERSION); // ver must equal "3.2.0" (or greater!)
  int major =
      ver[0] -
  ...
  std::cout << "OGLRenderer::OGLRenderer(): Maximum OGL version supported is "
            << major << "." << minor << "\n";

  if (major < 3) { // Graphics hardware does not support OGL 3! Erk...
    std::cout
        << "OGLRenderer::OGLRenderer(): Device does not support OpenGL 3.x!\n";
    wglDeleteContext(tempContext);
    return;
  }

  if (major == 3 &&
      minor < 2) { // Graphics hardware does not support ENOUGH of OGL 3! Erk...
    std::cout
        << "OGLRenderer::OGLRenderer(): Device does not support OpenGL 3.2!\n";
    wglDeleteContext(tempContext);
    return;
  }
  ...
  int attribs[] = {
      ...
      major, // TODO: Maybe lock this to 3? We might actually get an OpenGL 4.x
             // context...
      ...
      WGL_CONTEXT_CORE_PROFILE_BIT_ARB, // We want everything OpenGL 3.2
                                        // provides...
  ...
  if (!renderContext || !wglMakeCurrent(deviceContext, renderContext)) {
    std::cout
        << "OGLRenderer::OGLRenderer(): Cannot set OpenGL 3 context!\n"; // It's
                                                                         // all
                                                                         // gone
                                                                         // wrong!
  ...
  glClearColor(0.2f, 0.2f, 0.2f,
               1.0f); // When we clear the screen, we want it to be dark grey

  currentShader = 0; // 0 is the 'null' object name for shader programs...

  window.SetRenderer(
      this); // Tell our window about the new renderer! (Which will in turn
             // resize the renderer window to fit...)
}

/*
Destructor. Deletes the default shader, and the OpenGL rendering context.
*/
OGLRenderer::~OGLRenderer(void) { wglDeleteContext(renderContext); }
...
void OGLRenderer::UpdateShaderMatrices() {
  if (currentShader) {
    glUniformMatrix4fv(
        glGetUniformLocation(currentShader->GetProgram(), "modelMatrix"), 1,
        false, modelMatrix.values);
    glUniformMatrix4fv(
        glGetUniformLocation(currentShader->GetProgram(), "viewMatrix"), 1,
        false, viewMatrix.values);
    glUniformMatrix4fv(
        glGetUniformLocation(currentShader->GetProgram(), "projMatrix"), 1,
        false, projMatrix.values);
    glUniformMatrix4fv(
        glGetUniformLocation(currentShader->GetProgram(), "textureMatrix"), 1,
        false, textureMatrix.values);
    glUniformMatrix4fv(
        glGetUniformLocation(currentShader->GetProgram(), "shadowMatrix"), 1,
        false, shadowMatrix.values);
  }
}
...
void OGLRenderer::SetTextureFiltering(GLuint target, bool filter) {
  glBindTexture(GL_TEXTURE_2D, target);
  GLint param = filter ? GL_LINEAR : GL_NEAREST;
...
#ifdef OPENGL_DEBUGGING
void OGLRenderer::DebugCallback(GLenum source, GLenum type, GLuint id,
