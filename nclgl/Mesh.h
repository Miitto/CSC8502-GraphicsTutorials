...
public:
  ...
  unsigned int GetJointCount() const { return (unsigned int)jointNames.size(); }
  ...
  const Matrix4 *GetBindPose() const { return bindPose; }

  const Matrix4 *GetInverseBindPose() const { return inverseBindPose; }
