...
protected:
  ...
  void SetTextureRepeating(GLuint target, bool repeat);
  void SetTextureFiltering(GLuint target, bool filter);

  void StartDebugGroup(const std::string &s) {
    glPushDebugGroup(GL_DEBUG_SOURCE_APPLICATION, 0, (GLsizei)s.length(),
                     s.c_str());
  }

  void EndDebugGroup() { glPopDebugGroup(); }
